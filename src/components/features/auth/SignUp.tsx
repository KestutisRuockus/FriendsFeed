...
const SignUp = ({
  ...
  const validateInputs = () => {
    const validations: {
      ...
      { field: "password", error: "Password is required" },
      {
        ...
        condition: () => formState.password === formState.confirmPassword,
      },
  ...
  const handleTransitionToLogInComponent = (
  ...
  return (
    <div
      className={`bg-gradient-to-r from-extraColor to-bgColorSecondary md:w-1/2 w-full flex flex-col justify-center items-center py-16 md:rounded-l-xl md:rounded-none rounded-xl 
                      ${
                        isRegistrationComponent ? "opacity-100" : "opacity-0"
    ...
    >
      ...
      <form className="w-full flex lg:flex-row flex-col flex-wrap lg:items-start items-center gap-4 mt-12 m-auto">
        <label className="lg:w-1/3 w-2/3 lg:text-end text-start">
          Full name:
        </label>
        <input
        ...
        <div className="flex flex-wrap gap-4 lg:mt-0 mt-6 m-auto px-10">
          ...
          <Button
            text="Log In To Existing"
            fn={handleTransitionToLogInComponent}
          />
        </div>
